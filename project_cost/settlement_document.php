...
/**
 *  \file       htdocs/modulebuilder/template/Projectsettlement_document.php
 *  \ingroup    project_cost
 *  \brief      Tab for documents linked to Projectsettlement
 */
...
$extralabels = $extrafields->fetch_name_optionals_label('Projectsettlement');
...
$title=$langs->trans("Projectsettlement").' - '.$langs->trans("Files");
$help_url='';
...
if ($object->id)
{
	...
	dol_fiche_head($head, 'document', $langs->trans("Projectsettlement"), -1, 'Projectsettlement@project_cost');
	...
	// Object card
	// ------------------------------------------------------------
	$linkback = '<a href="' .dol_buildpath('/project_cost/Projectsettlement_list.php',1) . '?restore_lastsearch_values=1' . (! empty($socid) ? '&socid=' . $socid : '') . '">' . $langs->trans("BackToList") . '</a>';
	...
	//$relativepathwithnofile='Projectsettlement/' . dol_sanitizeFileName($object->id).'/';
	$relativepathwithnofile='Projectsettlement/' . dol_sanitizeFileName($object->ref).'/';
