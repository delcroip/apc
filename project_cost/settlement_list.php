 ...
 switch($action){
    ...
    case 'delete':
        if( $action=='delete' && ($id>0 || $ref!="")){
         $ret=$form->form_confirm(dol_buildpath('/project_cost/spread_card.php',1).'?action=confirm_delete&id='.$id,$langs->trans('DeleteProjectsettlement'),$langs->trans('ConfirmDelete'),'confirm_delete', '', 0, 1);
...
jQuery(document).ready(function() {
    ...
    $sql.= ' FROM '.MAIN_DB_PREFIX.'project_settlement as t';
    $sqlwhere='';
...
{
        $sqlcount='SELECT COUNT(*) as count FROM '.MAIN_DB_PREFIX.'project_settlement as t';
        if(!empty($sqlwhere))
